...
const rpwSpan = document.querySelector('#tip-repassword')
...
repassword.onblur = function () {
    ...
    if (value === password.value.trim()) {
        rpwSpan.textContent = '输入正确'
        rpwSpan.style.color = 'green'
    } else {
        rpwSpan.textContent = '输入错误'
        rpwSpan.style.color = 'red'
    }
}
...
submit.onclick = function (e) {
    // 判断两个数据有没有style
    if (!useSpan.style.color || !pswSpan.style.color || !repassword.style.color) return e.preventDefault()

    // 判断两个数据有没有style-color，颜色是不是绿色
    if (!(useSpan.style.color === 'green' && pswSpan.style.color === 'green' && repassword.style.color === 'green')) return e.preventDefault()
}
